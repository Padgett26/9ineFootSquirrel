...
if (filter_input ( INPUT_POST, 'userup', FILTER_SANITIZE_STRING )) {
	...
	$email = (filter_input ( INPUT_POST, 'email', FILTER_SANITIZE_EMAIL )) ? filter_input ( INPUT_POST, 'email', FILTER_SANITIZE_EMAIL ) : "";
	...
	if ($deluser == "1") {
		...
		$stmt = $db->prepare ( "SELECT COUNT(*) FROM users WHERE id != ? AND email = ?" );
		$stmt->execute ( array (
				$uid,
				$email
		) );
		...
		if ($ecount >= "1") {
			...
			if ($uid == "new") {
				...
				$stmt = $db->prepare ( "UPDATE users SET fName=?,email WHERE id=?" );
				$stmt->execute ( array (
						$fName,
						$email,
						$uid
...
if (filter_input ( INPUT_POST, 'merchUp', FILTER_SANITIZE_STRING )) {
	...
	$merchDescription = filter_input ( INPUT_POST, 'merchDescription', FILTER_SANITIZE_STRING );
	$merchSoldOut = (filter_input ( INPUT_POST, 'merchSoldOut', FILTER_SANITIZE_NUMBER_INT ) == '1') ? '1' : '0';
	$merchShowOrder = filter_input ( INPUT_POST, 'merchShowOrder', FILTER_SANITIZE_NUMBER_INT );
	...
	if ($delPic1 == '1') {
		$m2 = $db->prepare ( "SELECT merchPic1, merchPic1Ext FROM merch WHERE id=?" );
		$m2->execute ( array (
				$merchId
		) );
		$mrow2 = $m2->fetch ();
	...
	if ($delPic2 == '1') {
		$m2 = $db->prepare ( "SELECT merchPic2, merchPic2Ext FROM merch WHERE id=?" );
		$m2->execute ( array (
				$merchId
		) );
		$mrow2 = $m2->fetch ();
	...
	if ($merchId == 'new') {
		$mstmt1 = $db->prepare ( "INSERT INTO merch VALUES(NULL,?,'','0','jpg','0','jpg','0','1','','0','0','0')" );
		$mstmt1->execute ( array (
				$time
		) );
		$mstmt2 = $db->prepare ( "SELECT id FROM merch WHERE merchTitle=? ORDER BY id DESC LIMIT 1" );
	...
	$mstmt = $db->prepare ( "UPDATE merch SET merchTitle=?, merchDescription=?, merchSoldOut=?, merchShowOrder=?, ppCode=? WHERE id=?" );
	$mstmt->execute ( array (
			$merchTitle,
			$merchDescription,
			$merchSoldOut,
